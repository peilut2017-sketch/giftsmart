    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={onClose}>
      <div className="bg-white rounded-t-3xl w-full max-w-2xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-lg flex items-center gap-2"><Flag className="w-5 h-5 text-red-500" /> דווח על מוכר</h2>
...
function BuyModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={onClose}>
      <div className="bg-white rounded-t-3xl w-full max-w-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
        ...
        <div className="bg-gray-50 rounded-2xl p-4 space-y-1">
          ...
          <p className="text-sm text-gray-500">יתרה: ₪{listing.balance} · מחיר: <span className="text-green-600 font-bold">₪{listing.asking_price}</span></p>
        </div>
        ...
        <div className="space-y-2">
          ...
          {methods.length === 0 ? (
            ...
            methods.map((m, i) => (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs ${m.type === 'paypal' ? 'bg-blue-500' : m.type === 'bit' ? 'bg-purple-500' : m.type === 'paybox' ? 'bg-orange-500' : 'bg-teal-500'}`}>
        ...
        {selectedMethod && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-sm text-blue-800 space-y-1">
            ...
            <p className="font-mono text-base">{selectedMethod.value}</p>
          </div>
        )}
...
export default function ListingDetailPage() {
  ...
  useEffect(() => {
    ...
    // Fetch all listings and find the one
    setLoading(true)
    fetchListings().then(() => {
      setLoading(false)
    })
  ...
  // Update listing from context after fetch
  useEffect(() => {
