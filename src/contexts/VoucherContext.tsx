...
interface VoucherContextType {
  ...
  createGift: (voucherId: string, recipientEmail: string | null, message: string, sendAt: Date, codeOverride?: string, cvvOverride?: string) => Promise<string | null>
...
export function VoucherProvider({ children }: { children: ReactNode }) {
  ...
  async function createGift(voucherId: string, recipientEmail: string | null, message: string, sendAt: Date, codeOverride?: string, cvvOverride?: string): Promise<string | null> {
    if (!user) return null
    assertNotGuest()
    const token = Array.from(crypto.getRandomValues(new Uint8Array(24)))
    ...
    const { error } = await supabase.from('voucher_gifts').insert({
      ...
      token,
      // E2EE vouchers: the vault-decrypted code AND cvv travel on the gift row,
      // mirroring shared_voucher_tokens.code_override. Without them the recipient
      // would receive ciphertext encrypted with the sender's key — unusable.
      ...(codeOverride ? { code_override: codeOverride } : {}),
      ...(cvvOverride ? { cvv_override: cvvOverride } : {}),
